...
struct IRGenVisitor : public glu::gil::InstVisitor<IRGenVisitor> {
    ...
    {
        // Get the destructor function if one was generated by GILGen
        llvm::Function *dtorFn = nullptr;
        if (global->getDestructor() != nullptr) {
            dtorFn = createOrGetFunction(global->getDestructor());
        }

        // Generate the global variable
        if (global->getInitializer() == nullptr) {
            globalVarGen.generateGlobal(global, nullptr, dtorFn);
        } else {
            globalVarGen.generateGlobal(
                global, createOrGetFunction(global->getInitializer()), dtorFn
            );
        }

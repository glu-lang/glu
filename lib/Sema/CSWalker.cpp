...
#include <variant>
                                       ...
                                       namespace glu::sema
        {
            ...
            /// @brief Walks the AST to generate and solve constraints for
            /// expressions within a statement.
            class LocalCSWalker
                : public glu::ast::ASTWalker<LocalCSWalker, void> {
                ConstraintSystem _cs;
                glu::DiagnosticManager &_diagManager;
                glu::ast::ASTContext *_astContext;
            ...
            public:
                ...
                /// @brief preVisit method for all expressions to ensure they
                /// have a type before visiting them.
                void preVisitExprBase(glu::ast::ExprBase *node)
                {
                    // Ensure the expression has a type before visiting it
                    if (!node->getType()) {
                        // If the type is not set, we can create a type variable
                        // for it
                        auto typeVar
                ...
                /// @brief Visits a cast expression and emit an equality or
                ...
                /// @brief Visits a variable declaration and generates type
                /// constraints.
                void postVisitAssignStmt(glu::ast::AssignStmt *node)
                {
                    glu::ast::ExprBase *lhs = node->getExprLeft();
                ...
                /// @brief Visits a literal expression and generates type
                /// constraints.
                void postVisitLiteralExpr(glu::ast::LiteralExpr *node)
                {
                    ...
                    std::visit(
                        [&](auto &&arg) {
                            ...
                            if constexpr (std::is_same_v<T, llvm::APInt>) {
                                // Integer literal - default to signed 32-bit
                                // integer
                                defaultType
                                    = memoryArena.create<glu::types::IntTy>(
                                        glu::types::IntTy(
                                ...
                                // Float literal - default to 64-bit double
                                defaultType
                                    = memoryArena.create<glu::types::FloatTy>(
                                        glu::types::FloatTy(
                                            glu::types::FloatTy::DOUBLE
                                        )
                                    );
                                kind
                                ...
                                // Boolean literal
                                defaultType
                                    = memoryArena.create<glu::types::BoolTy>();
                                kind = ConstraintKind::ExpressibleByBoolLiteral;
                            } else if constexpr (std::is_same_v<
                                                     T, llvm::StringRef>) {
                                // String literal - create pointer to char type
                                defaultType
                                    = _cs.getScopeTable()->lookupType("String");
                                kind = ConstraintKind::
                    ...
                    // If we created a default type, create a defaultable
                    // constraint
                    if (defaultType) {
                        // Create a defaultable constraint between the node's
                        // type and the default type
                        auto constraint
                            = glu::sema::Constraint::createDefaultable(
                                _cs.getAllocator(), nodeType, defaultType, node
                            );
                    ...
                    auto constraint
                        = glu::sema::Constraint::createExpressibleByLiteral(
                            _cs.getAllocator(), nodeType, node, kind
                        );
                    _cs.addConstraint(constraint);
                }

                /// @brief Visits a return statement and generates type
                /// constraints.
                void postVisitReturnStmt(glu::ast::ReturnStmt *node)
                {
                    auto *expectedReturnType = _cs.getScopeTable()
                    ...
                    _cs.addConstraint(
                        Constraint::createConversion(
                            _cs.getAllocator(), returnType, expectedReturnType,
                            node
                        )
                    );
                }

                /// @brief Visits an if statement's condition and constrains it
                /// to boolean.
                ...
                /// @brief Visits a while statement's condition and constrains
                /// it to boolean.
                ...
                /// @brief Visits a ternary conditional expression and generates
                /// type constraints.
                void postVisitTernaryConditionalExpr(
                    glu::ast::TernaryConditionalExpr *node
                )
                {
                ...
                void postVisitStructMemberExpr(glu::ast::StructMemberExpr *node)
                {
                    ...
                    auto constraint = Constraint::createMember(
                        _cs.getAllocator(), ConstraintKind::ValueMember,
                        baseType, resultType, node, node
                    );
                    _cs.addConstraint(constraint);
                }
                ...
                void postVisitStructInitializerExpr(
                    glu::ast::StructInitializerExpr *
                )
                {
                }
                ...
                void postVisitBinaryOpExpr(glu::ast::BinaryOpExpr *node)
                {
                    ...
                    auto *expectedFnTy
                        = typesArena->create<glu::types::FunctionTy>(
                        ...
                        if (auto *fnDecl
                            ...
                            constraints.push_back(
                                Constraint::createBindOverload(
                                    _cs.getAllocator(), node->getType(), fnDecl,
                                    node
                                )
                            );
                        } else if (auto *varDecl
                                   = llvm::dyn_cast<glu::ast::VarLetDecl>(
                                       decl.item
                                   )) {
                            constraints.push_back(
                                Constraint::createConversion(
            ...
            private:
                ...
                expectedFnTypeFromCallExpr(glu::ast::CallExpr *node) const
                {
                    ...
                    return arena.create<glu::types::FunctionTy>(
                        argTypes, node->getType()
                    );
                }
            ...
            /// @brief Walks the AST to build scope tables and run local
            /// constraint systems. Runs the whole Sema pipeline.
            class GlobalCSWalker
                : public glu::ast::ASTWalker<GlobalCSWalker, void> {
            ...
            public:
                GlobalCSWalker(
                    glu::DiagnosticManager &diagManager,
                    glu::ast::ASTContext *context, ImportManager *importManager
                )
                    : _diagManager(diagManager)
                    , _context(context)
                    , _importManager(importManager)
                    , _scopeTableAllocator(
                          importManager->getScopeTableAllocator()
                      )
                {
                }
                ...
                void preVisitModuleDecl(glu::ast::ModuleDecl *node)
                {
                    _scopeTable = new (_scopeTableAllocator)
                        ScopeTable(node, _importManager);
                    UnresolvedNameTyMapper mapper(
                        *_scopeTable, _diagManager, _context
                    );
                ...
                void
                postVisitModuleDecl([[maybe_unused]] glu::ast::ModuleDecl *node)
                {
                ...
                void postVisitFunctionDecl(
                    [[maybe_unused]] glu::ast::FunctionDecl *node
                )
                {
                ...
                void postVisitCompoundStmt(
                    [[maybe_unused]] glu::ast::CompoundStmt *node
                )
                {
                    assert(
                        _scopeTable->getParent() && "Cannot pop global scope"
                    );
                    _scopeTable = _scopeTable->getParent();
                }
                ...
                void postVisitVarLetDecl(glu::ast::VarLetDecl *node)
                {
                    _scopeTable->insertItem(
                        node->getName(), node, node->getVisibility()
                    );
                }
            ...
            )
            {
                ...
                GlobalCSWalker(
                    diagManager, module->getContext(), &importManager
                )
                    .visit(module);
            }
            ...
            {
                // TODO: Make it actually fast by skipping unnecessary checks
                // (function bodies etc.)
                GlobalCSWalker walker(
                    diagManager, module->getContext(), importManager
                );
                walker.visit(module);
        ...
        } // namespace glu::sema->getFunctionDecl()
            ->getType()
            ->getReturnType();

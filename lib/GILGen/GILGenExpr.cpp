...
gil::Value GILGenExpr::visitCastExpr(CastExpr *expr)
{
    ...
    if (llvm::isa<types::IntTy>(sourceType)
        ...
        if (destIntTy->getBitWidth() < sourceIntTy->getBitWidth()) {
            // Integer truncation
            return ctx.buildIntTrunc(destGilType, sourceValue)->getResult(0);
            ...
            if (sourceIntTy->isSigned()) {
                ...
                // Unsigned extension
                return ctx.buildIntZext(destGilType, sourceValue)->getResult(0);
            }
        } else {
            // Same width but different signedness - use bitcast
            if (sourceIntTy->getSignedness() != destIntTy->getSignedness()) {
    ...
    if (llvm::isa<types::FloatTy>(sourceType)
        ...
        types::FloatTy *sourceFloatTy = llvm::cast<types::FloatTy>(sourceType);
        types::FloatTy *destFloatTy = llvm::cast<types::FloatTy>(destType);

        if (destFloatTy->getBitWidth() < sourceFloatTy->getBitWidth()) {
            // Float truncation
            return ctx.buildFloatTrunc(destGilType, sourceValue)->getResult(0);
        } else if (destFloatTy->getBitWidth() > sourceFloatTy->getBitWidth()) {
            // Float extension
            return ctx.buildFloatExt(destGilType, sourceValue)->getResult(0);
        } else {
            // Same type, no conversion needed
            return sourceValue;
        }
    ...
    if (llvm::isa<types::IntTy>(sourceType)
        ...
        // Int to Ptr
        return ctx.buildCastIntToPtr(destGilType, sourceValue)->getResult(0);
    }

    if (llvm::isa<types::PointerTy>(sourceType)
        && llvm::isa<types::IntTy>(destType)) {
        // Ptr to Int
        return ctx.buildCastPtrToInt(destGilType, sourceValue)->getResult(0);
    }

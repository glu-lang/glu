...
struct has_beforeVisit {
    template<typename U>
    static auto test(int) -> decltype(std::declval<U>().beforeVisit(std::declval<Args>()...), std::true_type{});
...
public:
    ...
    RetTy _visitType(TypeBase *type, ArgTys... args)
    {
        ...
        if constexpr (std::is_same_v<RetTy, bool> && has_beforeVisit<Impl, TypeBase*, ArgTys...>::value) {
